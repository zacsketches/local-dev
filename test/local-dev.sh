#!/bin/bash
# test/local-dev.sh - Test development environment for staticgen
...
echo -e "${GREEN}╔══════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  staticgen Test Environment         ║${NC}"
echo -e "${GREEN}╚══════════════════════════════════════╝${NC}"
...
(
  while true; do
    find src -type f -name '*.html' | entr -d -c bash -c '
      ...
      staticgen -src src -out site 2>&1 | grep -v "^$" || true
